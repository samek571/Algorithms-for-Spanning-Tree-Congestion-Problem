...
from stc_core import validate_tree, _relabel_graph_to_ints, _ensure_small_enough

Node = Hashable
Edge = Tuple[Node, Node]

MAX_EXACT_N = 18 #2^number so lets stay realistic within bounds of the atoms in universe (more than 20 is pretty bad)
...
#solve the decision problem stc(G) ≤ k using DP over states good[root][mask]
def _okamoto_decision_from_context(ctx: _OkamotoContext,G: nx.Graph, k: int, *, return_tree: bool = True) -> tuple[bool, Optional[nx.Graph]]:
    ...
    can I build a valid rooted tree on {root} ∪ mask under congestion bound k?
    A state is true in only two ways:
    1. attach one child subtree; pick a neighbor u of root inside mask, and if good[u][mask \ {u}] is true, then connect root to u
    2. split at the same root; split mask into two smaller parts sub and other; if both good[root][sub] and good[root][other] are true, then combine them under the same root
    """
    n,fullmask = ctx.n, ctx.fullmask

    #base case, mask=0 root is always feasable
    ...
    for size in range(1, n):
        for mask in ctx.masks_by_size[size]: #larger states depend on smaller, bottom up DP bysubset size
            ...
            for root in range(n): # for each subset mask dp tries every possible root that is not inside subset
                ...
                if mask & ctx.bit[root]:
                    continue
    
                #case1: look at neghbor u of root that lies inside mask, try to make u the root of the rest of the subtree
                #if good[u][mask \ {u}] is true, then you can connect root to u and obtain a feasible rooted tree for (root, mask)
                #(u,mask\{u}) implies (root,mask)
                found = False
                nbrs = ctx.neigh_mask[root] & mask #only eligible nodes to be attached to root
                while nbrs:
                    lsb = nbrs & -nbrs
                    ...
                    if ctx.cut_size[mask] <= k  and good[u][smaller]:
                        ...
                        if return_tree and choice_kind is not None and choice_arg is not None:
                            ...
                            choice_arg[root][mask] = u
                        found = True
                        break
                    nbrs ^= lsb #removes actual and goes to next
    
                if found: continue
                #if case 2 failed, dp asks: can the rooted tree for (root, mask) be obtained by
                #combining 2 smaller rooted trees that share the same root? HENCE
                #case 2: split mask into two non-empty proper parts (sub and other) and join them at root.
                sub = (mask - 1) & mask
                first_bit = mask & -mask #lsb
                while sub: #enum all subsets of masks
                    if sub & first_bit: #symmetry break...
                        other = mask ^ sub #xor gives other=mask\sub
                        if other and sub < other and good[root][sub] and good[root][other]:
                            good[root][mask] = 1
                            if return_tree and choice_kind is not None and choice_arg is not None:
                                choice_kind[root][mask] = 2
                                choice_arg[root][mask] = sub
                            break
                    sub = (sub - 1) & mask
    ...
    def _build_internal_helper(root_v: int, mask: int) -> List[tuple[int, int]]:
        ...
        all edges of a rooted tree on the vertex set {root_v} ∪ vertices(mask)
        where rooted tree is exactly the one whose existence was certified by good[root_v][mask] = True
        """
        if mask == 0: return [] #just root v, has no edges

        kind = choice_kind[root_v][mask] #how the state was made true
        arg = choice_arg[root_v][mask]

        #reconstruct child subtree and appends edge (root,u),
        if kind == 1: #attach one child rule, hence arg stores that child u
            u = arg #choose a neighbor u of root_v inside mask
            smaller = mask ^ ctx.bit[u] #(u was inside mask, in recurrence its peeled out of mask and promoted to be the child root of recursive subtree)
            edges = _build_internal_helper(u, smaller) #prove good[u][mask \ {u}], recursion to get all child subtree
            edges.append((root_v, u)) #conlcude good[root_v][mask]
            return edges
        ...
        if kind == 2: #split at root rule, hence arg has one side of the split
            sub = arg
            other = mask ^ sub
            return _build_internal_helper(root_v, sub) + _build_internal_helper(root_v, other) #concat edgelist of 2 recuriseive  {root_v} union other and {root_v} union sub
